...
public class StuInEd extends AppCompatActivity implements View.OnCreateContextMenuListener{
    ...
    public void stuin(View view) {
        ...
        if((!day1.matches("\\d+"))||day1.isEmpty()||Integer.parseInt(day1)<1||Integer.parseInt(day1)>31){takin = false;tvv1.setTextColor(0xffff0000);}//checks if vaccine 1 is on a valid day
        else{tvv1.setTextColor(0xff1B1B1F);}
        if((!month1.matches("\\d+"))||month1.isEmpty()||Integer.parseInt(month1)<1||Integer.parseInt(month1)>12) {takin = false;tvv1.setTextColor(0xffff0000);}//checks if vaccine 1 is on a valid month
        else{tvv1.setTextColor(0xff1B1B1F);}
        if((!year1.matches("\\d+"))||year1.isEmpty()){takin = false;tvv1.setTextColor(0xffff0000);}//checks if vaccine 1 is on a valid year
        else{tvv1.setTextColor(0xff1B1B1F);}
        if(v1l.getText().toString().isEmpty()){takin = false;tvv1.setTextColor(0xffff0000);}//checks if vaccine 1 has a location
        else{tvv1.setTextColor(0xff1B1B1F);}
        if((!day2.matches("\\d+"))||day2.isEmpty()||Integer.parseInt(day2)<1||Integer.parseInt(day2)>31||isodmytoymd(Integer.parseInt(day2),Integer.parseInt(month2),Integer.parseInt(year2))<isodmytoymd(Integer.parseInt(day1),Integer.parseInt(month1),Integer.parseInt(year1))) {takin = false;tvv2.setTextColor(0xffff0000);}//checks if vaccine 1 is on a valid day
        else{tvv2.setTextColor(0xff1B1B1F);}
        if((!month2.matches("\\d+"))||month2.isEmpty()||Integer.parseInt(month2)<1||Integer.parseInt(month2)>12) {takin = false;tvv2.setTextColor(0xffff0000);}//checks if vaccine 1 is on a valid month
        else{tvv2.setTextColor(0xff1B1B1F);}
        if((!year2.matches("\\d+"))||year2.isEmpty()){takin = false;tvv2.setTextColor(0xffff0000);}//checks if vaccine 1 is on a valid year
        else{tvv2.setTextColor(0xff1B1B1F);}
        if(v2l.getText().toString().isEmpty()){takin = false;tvv2.setTextColor(0xffff0000);}//checks if vaccine 1 has a location
        else{tvv2.setTextColor(0xff1B1B1F);}
        ...
        if(takin){
            ...
            Toast.makeText(this, "there was an issue with the reddened values", Toast.LENGTH_SHORT).show();
        }
    }
